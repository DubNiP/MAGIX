...
void FaseSegunda::criarChefoes() {
    ...
    entidades::personagens::MagoNegro* pM = new entidades::personagens::MagoNegro(Vector2f(900.f, 200.f), jog, Vector2f(3.f, -60.f));
    criaEntidade(pM);
    ...
    for (int i = 0; i < maxChefoes-1; i++) {
		int idx = dist2(rng);
        entidades::personagens::MagoNegro* pM = new entidades::personagens::MagoNegro(posicoesInimigos[idx], jog, Vector2f(3.f, -60.f));
        criaEntidade(pM);
        pM->setFaseAtual(this);
    }
}
